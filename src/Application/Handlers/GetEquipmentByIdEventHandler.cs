...
public class GetEquipmentByIdEventHandler : INotificationHandler<GetEquipmentByIdEvent>
{
    private readonly ILogger<GetEquipmentEventHandler> _logger;
    private readonly IMessageProducer _producer;
    private const string Topic = "equipment-events";
    public GetEquipmentByIdEventHandler(ILogger<GetEquipmentEventHandler> logger, IMessageProducer producer)
    {
        _logger = logger;
        _producer = producer;
    }

    public async Task Handle(GetEquipmentByIdEvent notification, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                EventType = nameof(GetEquipmentByIdEvent),
                notification.Id,
